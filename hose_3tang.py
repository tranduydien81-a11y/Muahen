...
Tang 3 (entry, su kien)      : gia pullback ve quanh SMA50 + RSI ngoc len
                               + RS manh hon VN-Index (loi suat N phien)
...
# --- Va lỗi thư viện vnstock trên môi trường lạ (GitHub Actions) ---
# Hàm nội bộ get_hosting_service() của vnstock bị UnboundLocalError khi
# không nhận diện được môi trường đang chạy (không phải Colab/Kaggle/local
# quen thuộc). Ghi đè hàm này về giá trị an toàn TRƯỚC khi gọi bất kỳ hàm
# vnstock nào — đây là né lỗi thư viện, không phải sửa logic của mình.
def _va_loi_vnstock_env():
    try:
        import vnstock.core.utils.env as _env
        _env.get_hosting_service = lambda: "Unknown"
        _env.is_colab = lambda: False
    except Exception as e:
        print(f"[!] Khong va duoc vnstock.core.utils.env ({e})")
    try:
        import vnstock.core.utils.client as _client
        _client.is_colab = lambda: False
    except Exception as e:
        print(f"[!] Khong va duoc vnstock.core.utils.client ({e})")

_va_loi_vnstock_env()

def _khai_vnstock_api_key():
    """Khai API key vnstocks.com de nang gioi han tu 20 (Khach) len
    60+ (Cong dong) request/phut. Doc key tu bien moi truong
    VNSTOCK_API_KEY (dat qua GitHub Secret, khong hardcode)."""
    key = os.environ.get("VNSTOCK_API_KEY")
    if not key:
        print("[VNSTOCK_API_KEY] Khong co — dung che do Khach (20 req/phut).")
        return
    try:
        import vnstock
        try:
            ...
            vnstock.change_api_key(api_key=key)
        print("[VNSTOCK_API_KEY] Da khai key thanh cong.")
    except Exception as e:
        print(f"[VNSTOCK_API_KEY] Loi khai key ({e}) — van tiep tuc voi che do Khach.")

_khai_vnstock_api_key()
...
CONFIG = {
    ...
    "rs_lookback": 20,               # tang 3d: so sanh loi suat N phien voi VN-Index
    "bat_rs_filter": True,           # ma phai outperform VN-Index trong rs_lookback phien

    "bat_regime_filter": True,

    "nghi_giua_2_ma_giay": 1,       # Guest = 20 req/phut. 3.5s/req -> an toan duoi nguong.
...
def tinh_chi_bao(df: pd.DataFrame, cfg: dict, vnindex_return_rs: float = None):
    n_dai = cfg["sma_dai"]
    if len(df) < n_dai + 10:
        return None
    ...
    # Tang 3d: suc manh tuong doi so voi VN-Index trong N phien gan nhat
    n_rs = cfg["rs_lookback"]
    ma_return_rs = None
    rs_manh = True  # mac dinh True neu khong bat filter hoac thieu du lieu index
    if len(close) > n_rs:
        ma_return_rs = float(close.iloc[-1] / close.iloc[-(n_rs + 1)] - 1)
    if cfg["bat_rs_filter"] and ma_return_rs is not None and vnindex_return_rs is not None:
        rs_manh = ma_return_rs > vnindex_return_rs
    ...
    return {
        ...
        "rs_20d_%": round(ma_return_rs * 100, 1) if ma_return_rs is not None else None,
        "trend": trend,
        "pullback_sma50": pullback,
        "rsi_vung": rsi_vung,
        "rsi_ngoc_len": rsi_ngoc,
        "rs_manh": bool(rs_manh),
        "entry": bool(trend and pullback and rsi_vung and rsi_ngoc and rs_manh),
    }


def kiem_tra_regime(cfg):
    """Tra ve (regime_ok, thong_diep, loi_suat_N_phien_cua_VNINDEX)."""
    if not cfg["bat_regime_filter"] and not cfg["bat_rs_filter"]:
        return True, "Regime filter dang TAT", None
    ...
    if df is None or len(df) < cfg["sma_dai"]:
        return True, f"Khong lay duoc {cfg['ma_chi_so']} — bo qua regime/RS filter", None
    close = df["close"].astype(float)
    sma200 = close.rolling(cfg["sma_dai"]).mean().iloc[-1]
    gia = close.iloc[-1]
    ok = (not cfg["bat_regime_filter"]) or (gia > sma200)

    n_rs = cfg["rs_lookback"]
    vnindex_return_rs = None
    if len(close) > n_rs:
        vnindex_return_rs = float(close.iloc[-1] / close.iloc[-(n_rs + 1)] - 1)

    msg = (f"{cfg['ma_chi_so']} {gia:,.1f} {'TREN' if gia > sma200 else 'DUOI'} "
          f"SMA200 ({sma200:,.1f})")
    if vnindex_return_rs is not None:
        msg += f" | loi suat {n_rs} phien: {vnindex_return_rs*100:.1f}%"
    return ok, msg, vnindex_return_rs
...
def main():
    ...
    regime_ok, regime_msg, vnindex_return_rs = kiem_tra_regime(cfg)
    print(f"\n[Tang 0] {regime_msg}")
    ...
    for i, ma in enumerate(danh_sach, 1):
        ...
        if df is not None:
            cb = tinh_chi_bao(df, cfg, vnindex_return_rs)
            if cb is not None:
                cb["ma"] = ma
                dong.append(cb)
        if i % 25 == 0:
    ...
    buoc = [
        ...
        (f"Tang 3d: RS manh hon VN-Index ({cfg['rs_lookback']} phien)", bang["rs_manh"]),
    ]
    ...
    cot = ["ma", "gia", "sma50", "sma200", "cach_sma50_%", "rsi14", "rs_20d_%",
           "pe", "pb", "gtgd_bq20_ty", "entry"]
